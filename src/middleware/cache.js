...
function verifyCache(key) {
    return (req, res, next) => {
        if(key === 'book' && cache.has('book') && (req.query.title  || req.query.address || req.query.genre)) {
            ...
            if(req.query.title) {
                books = books.filter(book => book.title.includes(req.query.title) || book.authorName.includes(req.query.title));
            }
